...
\begin{enumerate}[label=\roman*.]
    % \item $M(x) = \bar{x} + Z$ for $Z \sim \textrm{Uniform}(-1,1).$
...
\end{cases}.$$
    ...
    \end{cases}.$$
    % \item $M(x) = \bar{x}+Z$ for $Z\sim \textrm{Geo}(5/n)$, where  .
    % \item $M(x) = (\lceil 100 \bar{x}\rceil + Z)/100$ for $Z\sim \textrm{Geo}(4)$, where $\lceil y\rceil$ denotes the smallest integer greater than or equal to $y$.
...
\iffalse    Measure utility by the mean-squared residuals, as defined in Equation~(\ref{eqn:residuals}), but using your DP estimate for $\hat{\beta}$ instead of the OLS estimate.
    ...
    %A box-and-whisker plot is a useful way to show this, but you can explore visualization method
\fi   
    \iffalse
...
%\Snote{needs to be updated for this year}
