...
\begin{enumerate} %[label=\roman*.]
    ...
    \item The mechanism $M$ that takes a dataset $x\in \cS^{*}$ where each record $x_i\in \cS$ is an ascii string (e.g. the $i$'th individual's favorite surf break) and does the following:
        \begin{enumerate}
            ...
            \item If $n_s+\Lap(1/\varepsilon) \geq 2n/3$ (the top break wins by a supermajority), output $s$. 
